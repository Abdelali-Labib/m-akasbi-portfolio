...
import { FaEdit, FaTrash, FaPlus, FaExclamationTriangle, FaProjectDiagram, FaGithub, FaExternalLinkAlt, FaYoutube, FaPlay } from 'react-icons/fa';
...
const ManageProjects = () => {
  ...
  const getProjectImageUrl = (project) => {
    // For videos and playlists, thumbnails are in the 'thumbnail' field
    if ((project.type === 'video' || project.type === 'playlist') && project.thumbnail) {
      // YouTube thumbnails are full URLs, use them directly
      if (project.thumbnail.includes('img.youtube.com') || project.thumbnail.includes('i.ytimg.com')) {
        return project.thumbnail;
      }
      return project.thumbnail;
    }
    
    // For other projects, images are in the 'img' field
    if (project.img) {
      // If it's a YouTube thumbnail (contains youtube domains), use it directly
    ...
    // Fallback to imageUrl field if it exists
    if (project.imageUrl) {
      if (project.imageUrl.includes('img.youtube.com') || project.imageUrl.includes('i.ytimg.com')) {
        return project.imageUrl;
      }
      if (!project.imageUrl.startsWith('http')) {
        return getCloudinaryUrl(project.imageUrl);
      }
      return project.imageUrl;
    }
  ...
  const getProjectTypeInfo = (type) => {
    switch (type?.toLowerCase()) {
      ...
      case 'web':
      case 'website':
        return { icon: FaExternalLinkAlt, color: 'text-blue-500', bgColor: 'bg-blue-50 dark:bg-blue-900/20' };
      default:
  ...
  return (
    <div className="space-y-8">
      ...
      {!isFormVisible && (
        <div>
          <h2 className="text-xl font-semibold text-primary dark:text-light mb-6">Projets ({projects.length})</h2>
                    ...
                    <div className="relative h-48 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-900">
                      ...
                      <div className="absolute top-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <button 
                          ...
                          className="p-2 bg-white/90 dark:bg-gray-800/90 text-accent rounded-lg hover:bg-white dark:hover:bg-gray-800 transition-colors shadow-sm"
                          title="Modifier"
                        >
                        ...
                        <button
                          ...
                          className="p-2 bg-white/90 dark:bg-gray-800/90 text-red-500 rounded-lg hover:bg-white dark:hover:bg-gray-800 transition-colors shadow-sm"
                          title="Supprimer"
                        >
